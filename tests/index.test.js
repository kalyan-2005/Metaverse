...
describe("Authentication", () => {
  test("User is able to sign up only once", async () => {
    const username = "kalyan" + Math.random();
  ...
  test("Signin fails if the username and password are incorrect", async () => {
    ...
    // Then login the user with wrong credentials
    const response = await axios.post(BACKEND_URL + "/api/v1/signin", {
...
describe("Arena endpoints", () => {
  ...
  test("Delete endpoint is able to delete an element", async () => {
    ...
    let res = await axios.delete(`${BACKEND_URL}/api/v1/space/element`, {
      data: { id: response.data.space.elements[0].id },
      headers: {
        authorization: `Bearer ${userToken}`,
      },
    });
    ...
    expect(newResponse.data.space.elements.length).toBe(2);
  });
